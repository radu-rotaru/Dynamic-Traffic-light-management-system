...
class GNN(nn.Module):
    ...
    def forward(self, node_features, edges):
        ...
        # here we pass only the edge list, without weights 
...
class Agent:
    ...
    def train_gnn(self, num_episodes = 100, num_steps = 500):
        ...
        # get nodes of the network, meaning both junctions with traffic lights and junctions without
        ...
        # get all edges of the network, can be interpreted as road that connects two junctions
        edges = net.getEdges()

        # iterate through edges and get the junctions that it connects
        for edge in edges:
        ...
        for epoch in range(num_episodes):
            ...
            for step in range(num_steps):
                ...
                #get junction features, number of cars waiting at junctions and phase of traffic signal for this example
                for junction_id in self.gnn_nodes:
                    # these are the default features for junctions without traffic lights
                    cars_at_junction = 0
                    phase = -1
                ...
                # positive pairs are junctions that are connected
                for edge in self.junctions_connections:
                    positive_pairs.append((embeddings[edge[0]], embeddings[edge[1]]))
                
                # negative pairs are junctions that are not connected
                negative_pairs = []
                for i in range(len(self.gnn_nodes)):
                    # stop if we have the same number of pairs
                    if len(negative_pairs) == len(positive_pairs):
                        break
                    for j in range(i + 1, len(self.gnn_nodes)):
                        pair = (embeddings[i], embeddings[j])

                        # check that pair is not connected
                        if (node_id_to_index[self.gnn_nodes[i]], node_id_to_index[self.gnn_nodes[j]]) not in self.junctions_connections:
...
def run(train=True,model_name="model",epochs=50,steps=500,ard=False):
    ...
    for e in range(epochs):
        ...
        while step <= steps:
            ...
            #get junction features, phase and number of waitning vehicles
